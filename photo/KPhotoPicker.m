...
#import "KPMacros.h"
#import "KPFunctionView.h"
#import "NavView.h"

@interface KPhotoPicker ()<UICollectionViewDelegate, UICollectionViewDataSource>
...
- (void)getHeightVersionAllPhotos {
    ...
    [KPFunctionView getHeightVersionAllPhotos:^(PHFetchResult *allPhotos) {
...
- (void)getLowVersionAllPhotos {
    ...
    [KPFunctionView getLowVersionAllPhotos:^(ALAssetsGroup *group) {
        if (!_photoGroupArr) {
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([phoneVersion integerValue] >= 8) {
        ...
        [KPFunctionView getChoosePicPHImageManager:^(double progress) {
            ...
            if (!result) {
                ...
                if (cell.chooseStatus == NO) {
                    if ((_chooseArray.count+_choosePhotoArr.count)< _selectPhotoOfMax) {
                        ...
                        [KPFunctionView shakeToShow:signImage];
...
-(void)finishChoosePhotos:(UIButton *)finishbtn{
    ...
    if (![finishbtn.titleLabel.text isEqualToString:finishStr]&&_chooseArray.count) {
        [KPFunctionView finishChoosePhotos:^(NSMutableArray *myChoosePhotoArr) {
            
            NSMutableArray *images = [[NSMutableArray alloc] init];
            for (NSDictionary *dict in myChoosePhotoArr) {
                UIImage *image = dict[@"image"];
                [images addObject:image];
            }
            _selectPhotosBack(images);
            [self btnClickBack];
